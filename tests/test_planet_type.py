...
def test_generate_temperature(ice_type: PlanetType):
    for _ in range(RANDOM_TEST_RUNS):
        ...
        assert temp.value >= ice_type.min_temperature.value
        assert temp.value <= ice_type.max_temperature.value

def test_generate_radius(ice_type: PlanetType):
    for _ in range(RANDOM_TEST_RUNS):
        radius = ice_type.generate_radius()
        assert radius.value >= ice_type.min_radius.value
        assert radius.value <= ice_type.max_radius.value

def test_generate_density(ice_type: PlanetType):
    for _ in range(RANDOM_TEST_RUNS):
        density = ice_type.generate_density()
        assert density.value >= ice_type.min_density.value
        assert density.value <= ice_type.max_density.value